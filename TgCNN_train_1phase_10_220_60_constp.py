...
def train_1phase(args):
    ...
    label1='constp'
    path='/userhome/TgCNN_3D/3D_10_220_60_constp/'
    ...
    co=1e-4
    bo0=1.02
    rou_o=849/bo0   #underground density
    vo=3.
    cvis=0.
    crock=0.
    poro=0.2
    ...
    p_train=pp_.copy()  
    for i in range(nz):
        p_train[:,:,:,i]=p_train[:,:,:,i]-rou_o*gz*(z_ref+dz*i)/1e5
    
    tk_image= time_k_image2(t,logk_image,nx,ny,nz,args.n_logk)
    ...
    p_boun2_0=BHP-rou_o*gz*z_ref/1e5
    
    p_train=((p_train-p_boun2_0)/(p_boun1_0-p_boun2_0))
    ...
    tk_image_v= time_k_image2(t,logk_v_image,nx,ny,nz,args.n_logk_v)
    ...
    n_col=len(tk_train_v)
    n_train=len(p_train)
    
    N_batch=math.ceil(n_col/args.BATCH_SIZE2)
    BATCH_SIZE=math.ceil(n_train/N_batch)
    ...
    model = ConvNet_3d_hard_elu_10_220_60(p0,L_t,label1,100,1,False)
    ...
    for epoch in range(args.num_epoch):
        for i, data in enumerate(zip(train_loader1,train_loader2)):
            ...
            pt=tensor_time_diff2(pred_f_v,pred_f_v_,dt) 
            pt_image=cut_boun(pt,1)
            ...
            f1=torch.pow((pred_f-batch_y)*args.lam1,2).mean()
            
            j_in=(2*np.pi*dz/vo/np.log(r0/rw)*c*torch.exp(batch_x_v[:,1:2,:,:,:]))
            Q=(pred_f_v*(p_boun1_0-p_boun2_0)+p_boun2_0-Phi_w)*j_in/bo0
            
            fac2=Q/dx/dy/dz*bigt/co/(p_boun1_0-p_boun2_0)
            
            fac2_internal=cut_boun(fac2,1)
            residual=fac3*pt_image-fac1*xkpx-fac1*ykpy-fac1*zkpz+fac2_internal*well_image
            f2=torch.pow((residual)*args.lam2,2).mean()
            ...
            f4=torch.pow((no_flow_residual_x1)*args.lam4,2).mean()+\
                ...
                torch.pow((no_flow_residual_z2)*args.lam4,2).mean()
            ...
            f5=torch.pow((cor_res000)*args.lam5,2).mean()+\
                ...
                torch.pow((cor_res111)*args.lam5,2).mean()
            ...
            f6=torch.pow((line_res00x)*args.lam6,2).mean()+\
                ...
                torch.pow((line_res1y1)*args.lam6,2).mean()
